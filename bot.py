...
📞 Bog'lanish: @Aa_Asadbek | 932502719 | 775305505
...
5. Ro'yxat so'rasa → @Aa_Asadbek ga yo'nalt
...
8. Oxirida: "Boshqa savolingiz bo'lsa yozing! 😊" de
"""

START_MESSAGE = """Assalomu alaykum! 👋 Avtomentor yordamchi botiga xush kelibsiz!

Men quyidagilar haqida savollarga javob bera olaman:

🚗 Haydovchilik kurslari (B va BC toifa)
📝 Imtihonga tayyorlov (7 kunlik intensiv)
💻 Online kurs (avtomentorpro.uz)
💰 Narxlar, jadval va ro'yxatdan o'tish

Savolingizni yozing, yordam berishdan xursandman! 😊"""
...
async def handle_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message and update.message.text and update.message.chat.type == "private":
        print(f"Private xabar: {update.message.text[:30]}")
        reply = await get_ai_reply(update.message.from_user.id, update.message.text)
        await update.message.reply_text(reply)

    # Guruh xabari — faqat mention da javob ber
    elif update.message and update.message.text and update.message.chat.type in ["group", "supergroup"]:
        bot_username = application.bot_data.get("username", "")
        if bot_username and bot_username.lower() in update.message.text.lower():
            print(f"Guruh mention: {update.message.text[:30]}")
            # Mention ni o'chirib toza matn olish
            clean_text = update.message.text.replace(bot_username, "").strip()
            if not clean_text:
                clean_text = "Salom"
            reply = await get_ai_reply(update.message.from_user.id, clean_text)
            await update.message.reply_text(reply)
